...
Vector heliCenter = Vector(0, 0, 0);
Vector heliMove = Vector(-1, 0, 0);
 
Quaternion qua = Quaternion(1.0, 0.0, 0.0, 0.0);
 
Vector balloonPos = Vector(0, 70, 0);
Vector camPos = Vector(0, 0, 0);
...
void onDisplay( ) {
    ...
    gluLookAt(camPos.x , camPos.y, camPos.z, heliCenter.x, heliCenter.y, heliCenter.z, 0, 1, 0);
    ...
    {
        Vector p1 = Vector(8600, -400, -8600);
        Vector p2 = Vector(-8600, -400, -8600);
        Vector p3 = Vector(8600, -400, 8600);
        Vector p4 = Vector(-8600, -400, 8600);
    ...
    glPushMatrix();
    glTranslatef(heliCenter.x, heliCenter.y, heliCenter.z);
...
void onKeyboard(unsigned char key, int x, int y) {
    ...
    if (key == 32) {
        active[balldb] = true;
        pos[balldb] = heliCenter;
        dir[balldb] = heliMove * 20;
        balldb++;
        if (balldb == 20)
            balldb = 0;
    }
 
    Quaternion Direction = qua % Quaternion(0, Vector(-1, 0, 0)) % qua.getInverse();
    heliMove = Vector(Direction.GetAxis().x, Direction.GetAxis().y, Direction.GetAxis().z);
    heliMove.Normalize();
...
long startTime, oldTime, visibleTime, deltaTime, simTime = 0;
Vector oldpos[20], newpos[20], distance; 
 
float g = 0.00001;
float l = 0.01;
float airres = -0.000001;
float frope = 0.001;
Vector move = Vector(0, 0, 0);
Vector accel;
float ropeLength = 400;
...
void onIdle( ) {
    ...
    while (deltaTime > 0) {
        if (deltaTime > 20) simTime = 20;
        else simTime = deltaTime;
 
        animateIt = animateIt + 0.8 * simTime;
        heliCenter = heliCenter + heliMove * 0.08 * simTime;
 
        for(int i = 0; i < 20; i++){
            if(active[i] == true) {
 
                oldpos[i] = pos[i];
                dir[i] = dir[i] + Vector(0, -10, 0) * 0.005 * simTime;
        ...
        Vector pullForce = Vector(0, 0, 0);
        Vector pullDirection = (heliCenter - balloonPos);
        Vector L = Vector(0, 1, 0);
 
        if ( ropeLength - pullDirection.Length() <= 0) {
            pullForce = pullDirection.Normalize() * (ropeLength - pullDirection.Length()) * (-frope);
        }
 
        Vector resistForce = move * airres;
        move = (pullForce + resistForce + L * (l - g)) * simTime;
        balloonPos = balloonPos + move;
    }    
